    ...
    <section className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/50">{formatDateLabel(selectedDate)}</p>
        </div>
        <button
          type="button"
      ...
      <label className="block">
        ...
        <input
          ...
          className="mt-2 h-12 w-full min-w-0 rounded-[8px] border border-line bg-surface px-3 text-base outline-none focus:border-ocean"
        />
      </label>
      ...
      <div className="rounded-[8px] border border-line bg-surface p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {workouts.length === 0 && <span className="text-sm text-ink/50">暂无记录</span>}
