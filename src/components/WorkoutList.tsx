  ...
  return (
    <div className="space-y-3">
      {workouts.map((workout) => (
        <article key={workout.id} className="rounded-[8px] border border-line bg-white p-4">
              ...
              <div key={exercise.id} className="rounded-[8px] bg-mist px-3 py-2">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {exercise.sets.map((set, index) => (
                    <span key={set.id} className="rounded-[8px] bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-600">
          ...
          {workout.notes && <p className="mt-3 text-sm leading-6 text-slate-600">{workout.notes}</p>}
        </article>
      ))}
    </div>
  );
