...
src/
  ...
  utils/
    ...
    poseMovement.ts                # Movement rule engine (squat-class) — rep segmentation/classification/fatigue/QC from in-memory PoseFrame[]; parity-tested against the pose-analysis skill script; also builds the AI-coach DeepSeek messages
  storage/
  ...
  components/
    ...
    PoseMovementCard.tsx           # Movement report card — local rule-engine stats table + opt-in DeepSeek coach review (summary metrics only, never video/raw landmarks)
  views/
...
**Movement analysis** (`utils/poseMovement.ts` + `components/PoseMovementCard.tsx`, inside the pose card): after pose analysis, "分析动作" runs the local rule engine on the in-memory `PoseFrame[]` — knee/hip angle hysteresis segmentation (UP=150/DOWN=115, ±2-frame smoothing), per-rep metrics (min knee angle, hip angle at bottom, descent/ascent/bottom duration, trunk lean, hip-below-knee, L/R asymmetry), rep classification (`squat`/`hinge`/`crouch`/`partial` — only squats count; hinge = hip above knee or hip angle < knee angle at bottom), fatigue trend (ascent-time % change across ≥3 squats), and QC (detection rate/gaps). Thresholds mirror the user-level `pose-analysis` skill's validated script (`~/.agents/skills/pose-analysis/scripts/pose_report.mjs`); the TS port was parity-tested against it on real data. The "AI 教练点评" button (opt-in) sends only the computed summary JSON + condensed judging rules (`buildPoseCoachMessages`) to DeepSeek via the existing `chat()` service, reading the API key/body profile from storage at click time — never the video or raw landmarks.
