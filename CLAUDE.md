...
**Four tabs**, toggled via bottom nav (`App.tsx:608-613`):
...
- **Calendar** (`views/CalendarView.tsx`) — Week or month grid view. Dot indicators show workout (coral) and photo (ocean) presence per day. Period totals (sets, kcal) for the visible range at top. Below the grid: a donut chart. When the selected day has workouts, it shows the per-exercise reps distribution (`ExercisePieChart`) and clicking a slice reveals an `ExerciseLineChart`. When the selected day has no workouts, the donut is replaced by an all-history body-part distribution (`BodyPartPieChart`). Read-only workout list for the selected date.
...
- **Settings** (`views/SettingsView.tsx`) — Light/dark theme toggle, and data backup. Export supports either all data or a picked subset of dates; both produce a `fitlog-backup-YYYY-MM-DD.zip` (JSZip) containing `data.json`. Import accepts `.zip` or a raw `.json` and merges into the existing data.
...
src/
  ...
  views/
    ...
    SettingsView.tsx               # Theme toggle, body profile entry (BMI/BMR), ZIP/JSON export & import
public/
